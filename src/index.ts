		...
		this.server.tool(
			...
			"Create or update a Kaggle kernel (notebook/script) and start execution. Returns the kernel reference for status checking.",
			{
				...
				accelerator: z.enum([
					...
					"TpuV5E8",
					"TpuV6E8",
				]).default("none").describe("Kaggle accelerator (machineShape). Free for most users: NvidiaTeslaP100, NvidiaTeslaT4, NvidiaTeslaT4Highmem, TpuV6E8. Others (A100/L4/H100/etc.) require competition or admin access. Note: 'NvidiaTeslaT4x2' was removed from Kaggle's API; NvidiaTeslaT4Highmem is the current high-resource T4 option."),
		...
		this.server.tool(
			...
			"Push a Python script to Kaggle, wait for execution to complete, and return the output. Combines push + poll + output in one call.",
			{
				...
				accelerator: z.enum([
					...
					"TpuV5E8",
					"TpuV6E8",
				]).default("none").describe("Kaggle accelerator (machineShape). See kaggle_kernel_push for details on which are free vs. restricted."),
